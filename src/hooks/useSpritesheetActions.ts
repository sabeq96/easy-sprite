...
/** Every spritesheet mutation the UI can start, with its user-facing feedback built in. */
export function useSpritesheetActions() {
  return {
    create: (options: CreateSpritesheetOptions) => createSpritesheet(options),

    update: (id: string, edit: SpritesheetEdit) => updateSpritesheet(id, edit),
